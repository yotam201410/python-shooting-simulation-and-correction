...
class Ball(PhysicalObject, ABC):
    ...
    def calc_forces(self):
        ...
        self.add_force(
            -1 * velocity_direction * 0.5 * self.drag_constant * Constants.density_of_air * self.cross_section_area * (
        ...
        if abs(util.get_vector_magnitude(self.state.rotational_velocity)) > 0:
            ...
            self.add_force(np.cross(velocity_direction,
                                    axis_of_rotation) * 0.5 * self.lift_coefficient * Constants.density_of_air * self.cross_section_area * (
                                       velocity_magnitude ** 2))
